...
function screenToWorld(scoord,camx,camy,scale) {
  let x = Math.floor((scoord[0]-canvas.width/2)/scale+camx);
  let y = Math.ceil(camy-(scoord[1]-canvas.height/2)/scale);
  x = ringCoord(data.world.width, x);
  y = ringCoord(data.world.height, y);
  return [x,y];
}

function worldToScreen(wcoord,camx,camy,scale) {
  return [canvas.width/2+(wcoord[0]-camx)*scale,canvas.height/2-(wcoord[1]-camy)*scale];
}
...
function drawDebug(twoDContext,scale) {
  ...
  twoDContext.fillText("Selection: " + data.selection , 10, 80);
}
...
function drawHud(twoDContext,scale) {
  if(data.selection){
    let scoord = worldToScreen(data.selection,data.me.x,data.me.y,scale);
...
canvas.addEventListener('click', function(e) {
  ...
  if(data.selection && data.selection[0]==lastclick[0] && data.selection[1]==lastclick[1]) {
    data.selection=null;
  } else {
    data.selection=lastclick;
  }
}, false);
